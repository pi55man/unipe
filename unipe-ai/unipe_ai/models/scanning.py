...
def _vertical_alerts(
    ...
    for (src, dst), flows in groups.items():
        ports = {to_int(f.get("dst_port")) for f in flows}
        if len(ports) < cfg.min_ports_per_dst or not _looks_like_probing(flows, cfg):
            continue
        alerts.append(
            make_alert(
                ...
                severity="medium",
                confidence=ratio_confidence(len(ports), cfg.min_ports_per_dst, ceiling=5.0),
...
def _horizontal_alerts(
    ...
    for feat in probes:
        groups[(str(feat.get("src_ip", "")), to_int(feat.get("dst_port")))].append(feat)
